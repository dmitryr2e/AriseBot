...
async def _ask_model(client: httpx.AsyncClient, model: str, report_text: str) -> dict:
    ...
    if model.startswith("gemma"):
        ...
        generation_config = {"maxOutputTokens": 1024, "responseMimeType": "application/json"}
    ...
    resp = await client.post(_model_url(model), headers={"x-goog-api-key": config.GEMINI_API_KEY}, json=payload)
    try:
        resp.raise_for_status()
    except httpx.HTTPStatusError as exc:
        raise httpx.HTTPStatusError(f"{exc} | body={resp.text[:300]!r}", request=exc.request, response=exc.response) from exc
    return resp.json()
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        for model in models:
            try:
                ...
                stats = [s for s in parsed.get("stats", []) if s in config.STATS] or ["endurance"]
                verdict = str(parsed.get("verdict", "")).strip() or "ARISE приняла отчёт к сведению."
...
_INJECTION_PATTERNS: list[tuple[str, re.Pattern]] = [
    ("ignore_instructions_en", re.compile(r"ignore\s+(all\s+)?(the\s+)?previous\s+instructions", re.I)),
    ("disregard_above_en", re.compile(r"disregard\s+(all\s+)?(the\s+)?(above|previous)", re.I)),
    ("system_prompt_en", re.compile(r"system\s*prompt", re.I)),
    ("role_change_en", re.compile(r"\byou\s+are\s+now\b|\bact\s+as\b", re.I)),
    ("ignore_instructions_ru", re.compile(r"игнорируй\s+(вс[её]\s+)?(предыдущ\w*|прошл\w*)\s+инструкц\w*", re.I)),
    ("max_xp_ru", re.compile(r"(выдай|начисли|поставь)\S*\s+(мне\s+)?максимум\S*\s+(xp|опыт\w*|очков)", re.I)),
